  ...
  const NOTIFY_URL = 'notify_url';
	...
	public static function create_form( $order, $options, $return_url, $cancel_url ) {
		...
		$fields = array(
			...
			self::RETURN_TEXT => sprintf( __( 'Continue to %s', 'colabsthemes' ), get_bloginfo( 'name' ) ),
			self::RETURN_URL => $return_url,
			self::CANCEL_URL => $cancel_url,
			self::NO_SHIPPING => 1,
			self::NO_NOTE => 1,
      // self::NOTIFY_URL => add_query_arg( array('payment-gateway-api' => 'paypal'), home_url( '/' ) ),
      self::NOTIFY_URL => 'http://49ba64d7.ngrok.com',
		...
		$form = array(
			'action' => Colabs_Paypal_Gateway::get_request_url(),
			'name' => 'paypal_payform',
			'id' => 'create_listing',
		);
