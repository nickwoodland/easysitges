...
class Colabs_PayPal_Bridge{
	...
	public function __construct(){
		add_action( 'colabs_paypal_ipn_register_callbacks', array( $this, 'register_hooks' ) );
		
	}

	public function register_hooks(){

		Colabs_PayPal_Notifier::register( 'web_accept', array( $this, 'process_single' ) );

		Colabs_PayPal_Notifier::register( self::SUBSCRIBE_START, array( $this, 'process_subscription' ));
		Colabs_PayPal_Notifier::register( self::SUBSCRIBE_PAYMENT, array( $this, 'process_subscription' ) );
		Colabs_PayPal_Notifier::register( self::SUBSCRIBE_FAILED, array( $this, 'process_subscription' ) );
		Colabs_PayPal_Notifier::register( self::SUBSCRIBE_END, array( $this, 'process_subscription' ) );
		Colabs_PayPal_Notifier::register( self::SUBSCRIBE_EOT, array( $this, 'process_subscription' ));
	}
	...
	public function process_single( $response ){
		...
		$order = colabs_get_order( $item_number );
		if( !$order ){
			$error_data = array(
				'actual' => $item_number
			);
	...
	private function find_next_processable_subscription_order( $subscription_id ){
		...
		$posts = new WP_Query( array(
			...
			'post_status' => array( COLABS_ORDER_PENDING, COLABS_ORDER_FAILED ),
			'meta_query' => array(
				array(
