...
function colabs_security_check() {
    ...
    if (!isset($colabs_access_level) || $colabs_access_level=='') $colabs_access_level = 'read'; // if there's no value then give everyone access
...
if (get_option('colabs_admin_security') == 'true') {

    // comment out the below line to work with wpmu
    if ( !defined('DOING_AJAX') ) add_action('admin_init', 'colabs_security_check', 1);
